...
class DataIngestion:
    ...
    def get_required_interval(self):
        """
        The function to get split the different time intervals into many
        """
        ...
        try:
            ...
            logger.debeg(f"Prepared Interval : {intervals}")
    ...
    def convert_files_to_parquet(self,) -> str:
        """
        Convert all the json files to the parquet format
        """
        try:
    ...
    def retry_download_data(self, data, download_url: DownloadUrl):
        try:
            ...
            if len(wait_second) > 0:
                time.sleep(init(wait_second[0]) +2 )
    ...
    def download_data(self,download_url:DownloadUrl):
        """
        The function that downloads the data from the API based on the input and output params
        """
        try:
    ...
    def write_metadata(self):
        try:
            """
            This function is used to update the metadata information of which files are downloaded
            """
